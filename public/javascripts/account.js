    ...
    for (i = 0; i < data.length; i++) {
      $("#productOfOwner").append(`
        <tr>
          ...
          <td><a href="/merge/${data[i]}/${executefrom}" class="btn btn-primary">Merge</a><a href="/addaction/${data[i]}/${executefrom}" class="btn btn-info" style= "margin-left:5px;">Add Action</a><a href="/tranferOwnership/${data[i]}/${executefrom}" class="btn btn-danger" style= "margin-left:5px;">Tranfer Owner</a><a href="/setamount/${data[i]}" class="btn btn-success" id= "${i}" style= "margin-left:5px;">Set Amount</a></td>
        </tr>
      `);
