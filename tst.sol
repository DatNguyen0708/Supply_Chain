...
contract Product {
    ...
    event CreateContract(address proadd);
    event CreateContract1(address proadd1);
    ...
    function derive(bytes32 newProductsName, bytes32 unitChild, uint amountChild, uint ratioToChild, uint expirydateChild) notConsumed onlyOwner{ 
        ...
        CreateContract1(newProduct);     
    }

    // lay action
    function getAction(uint idx) constant returns (string, uint, uint, uint) {
